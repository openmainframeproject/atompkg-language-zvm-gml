...
module.exports =
class LanguageZvmGmlView
  constructor: (serializedState) ->
    # Create root element
    @element = document.createElement('div')
    @element.classList.add('language-zvm-gml')
    @element.tabIndex = "-1" # Ensure element can gain focus, so blur/etc work

    # Create message element
    message = document.createElement('div')
    message.classList.add('message')
    message.textContent = "This is the modal view from the language-zvm-gml package."
    ...
    item1.textContent = "The package has been activated; GML/Script files should have visible syntax highlighting."
    list.appendChild(item1)

    item2 = document.createElement('li')
    item2.textContent = "Run language-zvm-gml:Toggle again to remove this view."
  ...
  clickToHide: (panel) ->
    ...
    @element.addEventListener 'click',(event) =>
        # console.log 'LanguageZvmGmlView click event fired'
        @panel.hide()
    # Click OFF (away from) the content, and it disappears
    @element.addEventListener 'blur',(event) =>
        # console.log 'LanguageZvmGmlView blur event fired'
        @panel.hide()
